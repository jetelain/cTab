...
namespace CTab {
    ...
    let existingMapMarkers: { [name: string]: L.Layer } = {};
    ...
    function updateMapMarkers(msg: UpdateMapMarkersData): void {
        ...
        function process<TData extends { name: string }, TMarker extends L.Layer>(
            list: TData[],
            update: (data: TData, existing: TMarker, lastData: TData) => void,
            create: (data: TData) => TMarker | null
        ): void {
            list.forEach(function (data) {
                let marker = existingMapMarkers[data.name] as TMarker;
                if (marker) {
                    ...
                    if (newMarker) {
                        ...
                        update(data, newMarker, { pos: [] } as any);
        ...
        process(msg.icons,
            function (m, e: L.Marker, lastData) {
        ...
        process(msg.polylines,
            function (m, e: L.Polyline, lastData) {
